...
// Раньше свёрнутый вид оговорки резался через -webkit-line-clamp: 2 — по
// спецификации text-overflow (на нём и держится line-clamp) обрезка идёт
// посимвольно, а не по границе слова, поэтому текст иногда рвался прямо
// посреди слова ("диагноз…" вместо "диагноза…"). Текст оговорки статический
// (не зависит от пациента), так что вместо обрезки по пикселям — заранее
// выбранная граница по смыслу: первое предложение целиком, без "…".
const CAVEAT_PREVIEW =
  'Метод городских кварталов изначально применялся для проверки согласованности уже поставленного диагноза с частотными признаками ОЭРГ, а не для самостоятельной постановки диагноза.'
const CAVEAT_REST =
  ' Похожая по цели попытка классификации (нечёткая логика, одна патология: глаукома) дала не более 50% верных ответов, из-за малой и неоднородной выборки пациентов. Результат классификации в этом прототипе — это иллюстрация метода на реальных числовых признаках, а не клиническая рекомендация.'
        ...
        <aside className="sidebar">
          ...
          <section className="result">
            ...
            <p className="result__caveat">
              <strong className="result__caveat-label">Ограничения метода.</strong> {CAVEAT_PREVIEW}
              {caveatOpen && CAVEAT_REST}
            </p>
            <button
              type="button"
